...
export default {
  ...
  computed: {
    sensors () { return this.$store.state.sensors.sensors },
  ...
  methods: {
    editItem (item) {
      this.editedIndex = this.$store.state.sensors.sensors.indexOf(item)
    ...
    deleteItem (item) {
      ...
      removeAsset('sensor', item.id).catch(err => {
    ...
    save () {
      ...
      if (this.editedIndex > -1) {
        updateAsset('sensor', this.editedItem.id, this.editedItem)
        .catch(err => {
          console.log(err)
          return
        })
        ...
        // create new
        saveNewAsset('sensor', this.editedItem)
        .catch(err => {
          console.log(err)
          return
        })
